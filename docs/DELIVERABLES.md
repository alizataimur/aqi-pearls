...
| Evidence | `pytest tests/test_store_parity.py` (4 passed on Parquet; Hopsworks half **skipped** by that test file's own env-var check — see Outstanding); Parquet backend populated live by this session's backfill — **71,616 rows** (`capital`: 35,808, `lahore`: 35,808), 245 columns, 2022-08-04→2026-08-31, `data/feature_store/` |
| Outstanding | **Hopsworks is confirmed unavailable, not merely unconnected (ADR-034).** `HOPSWORKS_API_KEY`/`HOPSWORKS_PROJECT` are set in `.env` and GitHub Secrets, and were tested directly this session with a throwaway diagnostic using the `hopsworks` package's own `Connection.get_projects()` API: the key authenticates against Hopsworks' SaaS host and the backend responds (version 5.0.3) — proof the key is genuinely valid — but the account has **zero** projects, and creating one on this account's tier is a console action an API key cannot perform. `test_store_parity.py`'s Hopsworks half still shows as skipped above because that test checks `os.environ`, not `.env` (pydantic-settings reads `.env` internally without exporting it to the process environment) — it was never actually re-run against the real credentials, but the diagnostic's direct connection attempt makes the outcome known regardless: it would fail with "project not found," the same error `get_store()` now raises everywhere else in this repo when `FEATURE_STORE_BACKEND=hopsworks` is set. `HopsworksFeatureStore` is written against the documented SDK surface, is proven able to authenticate, and stays in the repo — the blocker is external and account-tier, not a bug or missing code |
